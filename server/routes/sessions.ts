...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      capture(req, { distinctId: createdBy, event: 'session_created', properties: { session_id: sessionId, airport_icao: airportIcao, is_pfatc: isPFATC, is_tutorial: isTutorial } });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (req.user?.userId) capture(req, { distinctId: req.user.userId, event: 'session_renamed', properties: { session_id: sessionId } });
...
router.post('/delete', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    capture(req, { distinctId: user.userId, event: 'session_deleted', properties: { session_id: sessionId, airport_icao: session.airport_icao } });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      capture(req, { distinctId: userId, event: 'session_deleted', properties: { session_id: oldestSession.session_id, airport_icao: oldestSession.airport_icao, auto_deleted: true } });
