...
router.get('/', createAuditLogger('ADMIN_API_LOGS_ACCESSED'), async (req, res) => {
    try {
        ...
        const statusCode = typeof statusCodeParam === 'string' ? parseInt(statusCodeParam) : undefined;
...
router.get('/stats', createAuditLogger('ADMIN_API_LOGS_STATS_ACCESSED'), async (req, res) => {
    try {
...
router.get('/:id', createAuditLogger('ADMIN_API_LOG_VIEWED'), async (req, res) => {
    try {
