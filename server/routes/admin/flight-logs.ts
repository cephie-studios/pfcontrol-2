...
router.get(
  '/',
  createAuditLogger('ADMIN_FLIGHT_LOGS_ACCESSED'),
  async (req, res) => {
    try {
      ...
      const general =
        typeof generalParam === 'string' ? generalParam : undefined;
      ...
      const action =
        ...
        validActions.includes(actionParam as (typeof validActions)[number])
          ? (actionParam as (typeof validActions)[number])
          : undefined;
      const session =
        typeof sessionParam === 'string' ? sessionParam : undefined;
      const flightId =
        typeof flightIdParam === 'string' ? flightIdParam : undefined;
      const date = typeof dateParam === 'string' ? dateParam : undefined;
      const text = typeof textParam === 'string' ? textParam : undefined;

      const data = await getFlightLogs(page, limit, {
        general,
        user,
        action,
        session,
...
router.get(
  '/:id',
  createAuditLogger('ADMIN_FLIGHT_LOG_VIEWED'),
  async (req, res) => {
    try {
...
router.post(
  '/reveal-ip/:id',
  createAuditLogger('FLIGHT_LOG_IP_REVEALED'),
