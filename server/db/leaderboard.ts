...
export async function getUserRank(userId: string, key: string): Promise<number | null> {
  try {
    const rank = await redisConnection.zrevrank(`leaderboard:${key}`, userId);
    return rank !== null ? rank + 1 : null;
  } catch (error) {
    console.error(`[Leaderboard] Failed to get rank for ${userId} on ${key}:`, error);
    return null;
  }
}

export async function getTopUsers(key: string, limit: number = 10): Promise<Array<{ userId: string; username: string; score: number }>> {
  try {
    const top = await redisConnection.zrevrange(`leaderboard:${key}`, 0, limit - 1, 'WITHSCORES');
    const entries = [];
