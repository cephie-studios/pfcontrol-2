...
export function playAudioWithGain(audioElement: HTMLAudioElement, volume: number): void {
...
export function playSoundWithSettings(
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            console.warn('Could not create audio element for:', soundMap[soundType], error);
            reject(error);
        }
    });
}
