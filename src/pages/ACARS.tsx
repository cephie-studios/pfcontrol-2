...
export default function ACARS() {
    ...
    useEffect(() => {
        ...
        const socket = createFlightsSocket(
            ...
            () => {},
            (error) => {
                console.error('Flight error (ACARS):', error);
                setError('Failed to submit ACARS/PDC request.');
                setIsSubmitting(false);
            }
        );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.callsign || !form.arrival || !form.aircraft_type) {
            setError('Please fill required fields (callsign, aircraft, arrival).');
            setIsSubmitting(false);
            return;
        }
        ...
        // Attach pdc request flag & pdc remarks if requested
        if (form.requestPDC) {
            payload.pdc = true;
            if (form.pdcRemarks) payload.pdc_remarks = form.pdcRemarks;
        }
        ...
        <div className="min-h-screen bg-gray-950 text-white">
            ...
            <div className="relative w-full h-56 md:h-72 overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 flex flex-col items-center p-6 md:p-10">
                    {session.airportIcao ? (
                        <h2 className="text-3xl sm:text-5xl font-extrabold text-blue-500 mb-6" style={{ lineHeight: 1.4 }}>
                            <span>{session.airportIcao}</span> - ACARS / PDC
                        </h2>
                    ) : (
                        'SUBMIT ACARS'
                    )}
                    ...
                    <div className="bg-green-900/30 border border-green-700 rounded-xl mb-8 overflow-hidden">
                        <div className="bg-green-900/50 p-4 border-b border-green-700 flex items-center">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-green-300 text-sm">
                                    Your request was submitted to ATC. A pre-departure clearance (PDC) will be issued if available.
                                </p>
                            </div>
                            <button
                                onClick={() => {
                        ...
                        <div className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white">{submittedFlight.aircraft || submittedFlight.aircraft_type}</p>
                                    </div>
                                    ...
                                    {submittedFlight.pdc && (
                                        <div>
                                            <span className="text-sm font-medium text-gray-400">PDC Requested:</span>
                                ...
                                <div className="space-y-3">
                                    ...
                                    {submittedFlight.pdc_remarks && (
                                        <div>
                                            <span className="text-sm font-medium text-gray-400">PDC Remarks:</span>
                                            <p className="text-white font-mono">{submittedFlight.pdc_remarks}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="mt-6 pt-4 border-t border-green-800">
                                <Button onClick={handleCreateAnother} className="flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white py-3 px-6 rounded-lg transition-colors">
                                    <PlusCircle className="h-5 w-5 mr-2" />
                                    Create Another ACARS/PDC
                                </Button>
                            </div>
                        </div>
                    </div>
                ...
                {!success && (
                    <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl border border-gray-800 shadow-xl overflow-hidden">
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <AircraftDropdown value={form.aircraft_type} onChange={(v) => handleChange('aircraft_type')(v)} />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <Dropdown value={form.flight_type} onChange={(v) => handleChange('flight_type')(v)} placeholder="IFR or VFR" options={[{ label: 'IFR', value: 'IFR' }, { label: 'VFR', value: 'VFR' }]} />
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <AirportDropdown value={form.departure} onChange={(v) => handleChange('departure')(v)} disabled />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <AirportDropdown value={form.arrival} onChange={(v) => handleChange('arrival')(v)} />
                                    </div>
                            ...
                            {/* PDC Request */}
                            <div className="p-4 bg-gray-800/30 rounded-md border border-gray-700">
                                <label className="flex items-center justify-between">
                                    <div className="flex items-center">
                                        <MapPinCheck className="h-5 w-5 mr-3 text-gray-300" />
                                        <div>
                                            <div className="text-sm font-medium text-gray-300">Request PDC (Pre-Departure Clearance)</div>
                                            <div className="text-xs text-gray-400">If enabled, ATC may issue a PDC for your flight</div>
                                        </div>
                                    </div>
                                    <div>
                                        <label className="inline-flex relative items-center cursor-pointer">
                                            <input type="checkbox" checked={!!form.requestPDC} onChange={(e) => handleChange('requestPDC')(e.target.checked)} className="sr-only peer" />
                                            <div className="w-11 h-6 bg-gray-600 peer-focus:outline-none rounded-full peer peer-checked:bg-blue-600 transition-colors" />
                                        </label>
                                    </div>
                                </label>
                                ...
                                {form.requestPDC && (
                                    <div className="mt-3">
                                        <label className="text-sm text-gray-300 mb-1 block">PDC Remarks (optional)</label>
                                        <input
                                            type="text"
                                            value={form.pdcRemarks}
                                            onChange={(e) => handleChange('pdcRemarks')(e.target.value)}
                                            placeholder="e.g. request SID / fuel status / taxi brief"
                                            className="w-full pl-4 p-2 bg-gray-800 border-2 border-blue-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                                        />
                                    </div>
                                )}
                            </div>
