...
export default function Create() {
  ...
  const [selectedArrivalRunway, setSelectedArrivalRunway] = useState<string>("");
  const [networkKind, setNetworkKind] = useState<NetworkSessionKind>("standard");
  ...
  const handleDeleteOldestSession = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to delete oldest session");
    } finally {
  ...
  const handleCreateSession = async () => {
    ...
    if (sessionLimitReached) {
      const maxSessions = user?.isAdmin || user?.isTester ? 100 : 50;
      setError(`Session limit reached. You can create up to ${maxSessions} sessions.`);
      return;
    }
    ...
    try {
      const effectiveKind: NetworkSessionKind = startTutorial ? "pfatc" : networkKind;
      ...
      try {
        const landingRunways = selectedArrivalRunway ? [selectedArrivalRunway] : [selectedRunway];
        ...
        console.warn("Failed to generate ATIS during session creation:", atisError);
      }

      const showAtisReminder =
        (effectiveKind === "pfatc" || effectiveKind === "advanced_atc") && atisResponse?.atisText;
      if (showAtisReminder) {
        setCreatedSession({
          sessionId: newSession.sessionId,
          accessId: newSession.accessId,
          atisText: atisResponse?.atisText || "",
          networkSessionKind: effectiveKind === "advanced_atc" ? "advanced_atc" : "pfatc",
        });
    ...
    <div className="min-h-screen bg-gray-950 text-white relative">
        ...
        <div className="bg-gray-900/70 backdrop-blur-md border border-gray-800 rounded-4xl p-6 space-y-6 shadow-2xl">
          ...
          >
            <div className="flex items-center">
              ...
              />
              <span>
                Sessions: {sessionCount}/{user?.isAdmin || user?.isTester ? 100 : 50}
            ...
            {sessionLimitReached && (
              ...
              >
                ...
                <span>{isDeletingOldest ? "Deleting..." : "Delete Oldest"}</span>
              </Button>
            )}
          </div>
          ...
          <div id="arrival-runway-dropdown" className="space-y-2">
            <label className="block text-sm font-medium text-gray-300">
              Select Arrival Runway <span className="text-gray-500">(Optional)</span>
            </label>
            <RunwayDropdown
              airportIcao={selectedAirport}
            ...
            <div className="flex flex-row gap-4 w-full">
              ...
              <div className="flex-1">
                <Checkbox
                  ...
                  checked={startTutorial ? false : networkKind === "advanced_atc"}
                  onChange={(checked) => {
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm">
                    <p className="text-blue-300">
                      All submitted flights will be publicly viewable on the Network Overview page.
                      Flights are shared between sessions, allowing controllers to see the arrivals
                      and traffic of other open sessions.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {showAtisReminderModal && createdSession && user && (
        <AtisReminderModal
          ...
          airportIcao={selectedAirport}
          airportName={airports.find((a) => a.icao === selectedAirport)?.name || selectedAirport}
          airportControlName={
            airports.find((a) => a.icao === selectedAirport)?.controlName || selectedAirport
          }
          airportAppFrequency={(() => {
          ...
          airportFrequencyType={(() => {
            ...
            for (const key of order) {
              const fromAirport = airportObj?.allFrequencies?.[key];
              const fromFreqs = freqObj?.[key];
              if ((fromAirport && fromAirport !== "n/a") || (fromFreqs && fromFreqs !== "n/a"))
                return key;
            }
            return "APP";
          })()}
